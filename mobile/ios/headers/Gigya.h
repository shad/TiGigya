...
// Consts
static NSString * const GSGigyaSDKVersion = @"iOS_2.17";
...
typedef NS_ENUM(NSInteger, GSErrorCode) {
    ...
    /** This operation needs a user permission and it was not requested. You may use the method socialize.requestPermissions to request the user permission. 
     After gaining user permission you may retry to execute this operation. */
...
typedef void(^GSPermissionRequestResultHandler)(BOOL granted, NSError *error);
...
/** @name Handling the Gigya Session */
...
/*!
 Gets the delegate that response to the session events.
 @see GSSessionDelegate
 */
+ (id<GSSessionDelegate>)sessionDelegate;

/*!
 Sets the delegate that response to the session events. This delegate will be called regardless of the handler blocks passed to the different methods.
 @see GSSessionDelegate
 */
...
/*!
 ...
 | facebookReadPermissions    | NSString    | A comma delimited list of Facebook extended permissions to request from the user **when using [native login](http://developers.gigya.com/035_Mobile_SDKs/010_iPhone#Adding_Facebook_Native_Login)**. This parameter gives the possibility to request extended permissions in addition to the permissions that Gigya is already requesting. Please refer to Facebook's [permissions](https://developers.facebook.com/docs/reference/login/#permissions) documentation for the complete list of read permissions. The default value of this parameter is "basic_info,email". Note: you should only include read permissions, otherwise Facebook will fail the login. |
 | facebookExtraPermissions   | NSString    | A comma delimited list of additional publish permissions to be requested from the user. Used only **when not using native Facebook login**. When using native login, use the <requestFacebookPublishPermissions:responseHandler:> after opening the session. |
 ...
 | sessionExpiration          | NSNumber    | A time interval which defines the time in seconds that Gigya should keep the login session valid for the user. If this parameter is not specified, the session will be valid forever. |
 ...
 @param handler A completion handler that will be invoked when the login process is finished. The handler should have the signature `(GSUser *user, NSError *error)`. If the login was succesful, the `error` parameter will be `nil`. Otherwise, you can check the `error.code` (see `GSErrorCode`) or `error.userInfo` to learn why it failed.
 */
+ (void)loginToProvider:(NSString *)provider
...
/*!
 ...
 @param handler A completion handler that will be invoked when the logout process is finished. The handler should have the signature `(GSResponse *response, NSError *error)`. If the call to socialize.logout was succesful, the `error` parameter will be `nil`. Otherwise, you can check the `error.code` (see `GSErrorCode`) or `error.userInfo` to learn why it failed.
 */
...
/*!
 ...
 @param handler A completion handler that will be invoked when adding the connection is finished. The handler should have the signature `(GSUser *user, NSError *error)`. If the connection was added succesfuly, the `error` parameter will be `nil`. Otherwise, you can check the `error.code` (see `GSErrorCode`) or `error.userInfo` to learn why it failed.
 */
+ (void)addConnectionToProvider:(NSString *)provider
                     parameters:(NSDictionary *)parameters
              completionHandler:(GSUserInfoHandler)handler;
...
/*!
 ...
 @param provider A provider name string.
 @param handler A completion handler that will be invoked when the removal process is finished. The handler should have the signature `(GSUser *user, NSError *error)`. If the call to socialize.removeConnection was succesful, the `error` parameter will be `nil`. Otherwise, you can check the `error.code` (see `GSErrorCode`) or `error.userInfo` to learn why it failed.
 */
+ (void)removeConnectionToProvider:(NSString *)provider
                 completionHandler:(GSUserInfoHandler)handler;

/** @name Requesting Facebook Publish Permissions */

/*!
 Asks the user for Facebook publish permissions. This method works only with [Facebook native login](http://developers.gigya.com/035_Mobile_SDKs/010_iPhone#Adding_Facebook_Native_Login), and must be called after the user logged in.
 ...
 @param handler A completion handler that will be invoked when the process if finished. The handler should have the signature `(BOOL granted, NSError *error)`. If the `granted` parameter is `NO`, you can check the `error` parameter to learn why it failed.
 */
+ (void)requestFacebookPublishPermissions:(NSString *)permissions
                          responseHandler:(GSPermissionRequestResultHandler)handler;

/** @name Delegating URL Actions */

/*!
 Handles URLs being opened by your AppDelegate. If Facebook or Google+ [native login](http://developers.gigya.com/035_Mobile_SDKs/010_iPhone#Adding_Facebook_Native_Login) is being used, this method must be called and returned from your AppDelegate's `application:handleOpenURL:` or `application:openURL:sourceApplication:annotation:` method.
 ...
 @returns Returns `YES` if the Facebook or Google+ SDKs handled this URL.
 */
