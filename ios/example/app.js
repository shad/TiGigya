...
showLoginProvidersButton.addEventListener('click', function(e) {
    ...
    module.showLoginProvidersDialog({
        ...
        success: function(e) {
            label.text = 'logged in as ' + e.user.firstName +
                ' ' + e.user.lastName;
            Ti.API.info(
                "success: showLoginProvidersDialog: " +
                JSON.stringify(e));
        },
        failure: function(e) {
            label.text = 'login failure: ' + e.error;
            Ti.API.info(
                "failure: showLoginProvidersDialog: " +
                JSON.stringify(e));
        }
    });
});
...
sessionButton.addEventListener('click', function(e) {
    ...
    label.text = session ? "token=" + session.token + "; isValid=" +
...
addConnectionProvidersButton.addEventListener('click', function(e) {
    if (module.session && module.session.isValid) {
        module.showAddConnectionProvidersDialog({
            ...
            success: function(e) {
                label.text = "added connection provider: " +
                    JSON.stringify(e);
            },
            failure: function(e) {
                label.text =
                    "failed to add connection provider: " +
                    JSON.stringify(e);
            }
        });
    }
